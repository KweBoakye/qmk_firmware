...
#define WIDTH OLED_DISPLAY_WIDTH          // for vertical displays
#define HEIGHT OLED_DISPLAY_HEIGHT                // for vertical displays
 ...
 * Wave Parameters
 */
#define OCEAN_LINE 6      // Line you want to render the ocean starting at (best at oled_max_lines() - 2)
 ...
 * Star Parameters
 */
#define STARS_PER_LINE 16        // number of stars per line (for a display that is 128x32, this would be 4 stars spread out evenly over the 32byte width, one every 8 bytes)
#define NUMBER_OF_STAR_LINES 8 // number of lines to fill up with stars (for a display that is 128x32, 16 bytes are filled with the ocean animation, so that leaves 112 pixels left over. The number of lines depends on your OLED_FONT_HEIGHT)
 ...
 * Island Parameters
 */
#define ISLAND_LINE 4       // line that the island starts at. Island is 2 lines tall
#define ISLAND_COLUMN 0        // column that the island starts at
#define SECOND_ISLAND
#ifdef SECOND_ISLAND
       #define ISLAND_COLUMN_2 16
#endif
