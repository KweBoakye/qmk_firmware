...
static void animate_waves(void) {
    starry_night_wave_frame_width_counter = decrement_counter(starry_night_wave_frame_width_counter, ((WIDTH - 1)/ 4) );  // only 3 frames for last wave type
  ...
  for ( int j = 0; j < WIDTH; j = j + (WIDTH/4)){
    for (int i = 0; i < (WIDTH/4); ++i) {
        uint16_t offset     = OCEAN_LINE * WIDTH + i + j;
        uint8_t  byte_index = starry_night_wave_frame_width_counter + i;
        if (byte_index >= (WIDTH/4)) {
            byte_index = byte_index - (WIDTH/4);
        }
...
#ifdef SECOND_ISLAND
         oled_set_cursor(ISLAND_COLUMN_2 + 3, ISLAND_LINE);
        oled_write_raw_P(islandRightTop[frame], 14);
        oled_set_cursor(ISLAND_COLUMN_2 + 0, ISLAND_LINE + 1);
        oled_write_raw_P(islandLeft, 18);
        oled_set_cursor(ISLAND_COLUMN_2 + 3, ISLAND_LINE + 1);
        oled_write_raw_P(islandRightBottom[frame], 14);
        #endif
    }
