...
static void write_charybdis_config_to_eeprom(charybdis_config_t* config) { eeconfig_update_user(config->raw); }
...
static void maybe_update_pointing_device_cpi(charybdis_config_t* config) {
    if (config->is_dragscroll_enabled) {
        pmw3360_set_cpi(CHARYBDIS_DRAGSCROLL_DPI);
    } else if (config->is_sniping_enabled) {
        pmw3360_set_cpi(get_pointer_sniping_dpi(config));
    } else {
        pmw3360_set_cpi(get_pointer_default_dpi(config));
    }
}
...
void pointing_device_init_trackball(void) { maybe_update_pointing_device_cpi(&g_charybdis_config); }
...
#    ifdef CONSOLE_ENABLE
    dprintf("(charybdis) process_record_trackball: config = {\n"
            "\traw = 0x%04X,\n"
            "\t{\n"
...
bool process_record_trackball(uint16_t keycode, keyrecord_t* record) {
    if (!process_record_trackball(keycode, record)) {
...
void eeconfig_init_trackball(void) {
