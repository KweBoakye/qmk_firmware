...
if (is_keyboard_left()) {
    ...
    analog_joystick_report = analog_joystick_pointing_device_task(mouse_report);
     ...
     // mouse_report.y = pointing_device_movement_clamp((int16_t) -pmw3360_report.y + -cirque_pinnacle_report.x);
      ...
      mouse_report.x = pointing_device_xy_clamp((clamp_range_t) analog_joystick_report.x + cirque_pinnacle_report.y);
      mouse_report.y = pointing_device_xy_clamp((clamp_range_t) analog_joystick_report.y + -cirque_pinnacle_report.x);
       mouse_report.h = pointing_device_xy_clamp((clamp_range_t) analog_joystick_report.h + cirque_pinnacle_report.v);
      mouse_report.v = pointing_device_xy_clamp((clamp_range_t) analog_joystick_report.v + -cirque_pinnacle_report.h);
